...
2. **WeaponControllerBase** (`src/acsdg_c2/include/acsdg_c2/weapon_controller_base.hpp` + `.cpp`). Abstract C++ class owning the topic plumbing, watchdog, return-to-home, and 20 Hz timer. Two virtuals: `computePursuitCmd` (write WORLD-frame velocity) and `onKill` (decide outcome at kill-radius). `AnvilControllerNode` and `CoyoteControllerNode` shrunk from ~270 lines each to ~80 lines each by lifting the shared 80%. Phase 3's DroneHunter F700 lands by adding one ~50-line subclass — no template chain, no copy-paste. The base class also takes an explicit `gz_model_kind` ctor parameter (Anvil passes "interceptor", Coyote passes "coyote") so future weapons aren't subject to fragile string surgery on the topic prefix.
...
4. **ToI `eff_speed` clamp** (`src/acsdg_c2/acsdg_c2/weapons/{base,anvil,coyote}.py`). Added `_clamp_eff_speed()` to the WeaponSystem base class with module-level constant `_EFF_SPEED_FLOOR_FRAC = 0.5`. Both `time_to_intercept` methods call this helper. SHAHED-class stern-aspect targets now yield a finite, seconds-unit ToI that the Hungarian cost matrix can rank correctly. Closes N-C.

5. **Test additions.** `test_fleet.py` (12 tests: validator + SDF agreement + bridged_models). `test_launch_dryrun.py` (2 tests: launch description has one controller per FLEET slot, with home parameters). `test_coyote_integration.py` (2 tests: dispatch routing + MISS-ack handling, in-process rclpy harness). Anvil + Coyote ToI clamp tests in their existing files. Test count grew from ~60 to **79 passing**. Closes M-1 and I-2 regression-test items.

6. **Launch refactor.** `c2.launch.py` iterates `FLEET` instead of using a hand-coded if/else ladder. Per-class launch parameters lifted to `WeaponSystem.launch_parameters()` method (default empty list); Coyote overrides to return `pkill_small_quad` + `rng_seed`. Adding DroneHunter is one append to FLEET + one override of `launch_parameters()` — no edit to launch.py.

7. **MAVROS dependency dropped.** The Anvil controller's pre-existing `mavros_msgs/srv/SetMode` client + setpoint publisher were demonstrably dead-on-arrival in simulation (no MAVROS node, no PX4 SITL launched, no subscribers). Dropped during the WeaponControllerBase refactor. Build manifest cleaned up — `find_package(mavros_msgs REQUIRED)` and `<depend>mavros_msgs</depend>` (and `<depend>mavros</depend>` from acsdg_gazebo) removed. If real-hardware bringup is later needed, MAVROS plumbing re-adds in the relevant subclass — clean re-add path.

**Live verification.** Phase 2 demo procedure (`ros2 launch acsdg_bringup acsdg_full.launch.py headless:=false` + `ros2 topic pub --times 5 -r 0.5 /mission/wave_trigger std_msgs/Bool '{data: true}'`) reproduces 4 NEUTRALISED, 0 BREACHED — modulo occasional `[FRAG-FUZE MISS]` lines on 5–8 m near-miss engagements (~40% of outer-ring rolls at `pkill=0.60`), followed by C2 reassignment to an Anvil. Demonstrates adaptive heterogeneous-fleet dispatch in the live demo.
